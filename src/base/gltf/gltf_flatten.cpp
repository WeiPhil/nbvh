...
glm::mat4 read_node_transform(const tinygltf::Node &n)
{
    ...
    if (!n.matrix.empty()) {
        ...
        if (!n.rotation.empty()) {
            const glm::quat rot = glm::quat(n.rotation[3], n.rotation[0], n.rotation[1], n.rotation[2]);
            transform           = glm::mat4_cast(rot) * transform;
        }
        if (!n.translation.empty()) {
            const glm::mat4 translate = glm::translate(glm::vec3(n.translation[0], n.translation[1], n.translation[2]));
                       ...
                       std::vector<tinygltf::Node> &flattened)
{
    ...
    flat.matrix.clear();
    std::copy(glm::value_ptr(transform), glm::value_ptr(transform) + 16, std::back_inserter(flat.matrix));
...
void flatten_gltf(tinygltf::Model &model)
{
    if (gltf_is_single_level(model)) {
        ...
        logger(LogLevel::Warn, "Scene is being flattened scene");
    }
